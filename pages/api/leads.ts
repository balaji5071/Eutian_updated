...
import { Lead, PLAN_OPTIONS } from '@/shared/schema';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'POST') {
      const { name, email, region, message, phone, whatsapp, websiteType, plan, source } = req.body ?? {};
    ...
    if (req.method === 'PATCH') {
      const { id, status, notes, dealValue, currency, paymentStatus, followUpDate, source } = req.body ?? {};
      if (!id) return res.status(400).json({ ok: false, error: 'Missing id' });
      
      const updates: Partial<Lead> = {};
      if (status) {
        if (!['new', 'contacted', 'closed', 'won'].includes(status)) {
      ...
      const result = await leads.updateOne({ _id: new ObjectId(id) }, { $set: updates });
