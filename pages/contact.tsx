	...
	const onSubmit = async (data: ContactFormValues) => {
		...
		try {
			...
			if (!res.ok) {
				...
				const errorMessage = payload?.error || 'Failed to submit your inquiry';
				
				// Provide helpful error messages based on status code
				if (res.status === 500) {
					throw new Error('Server error. Please try again in a few moments or contact us directly via phone/WhatsApp.');
			...
			let errorDescription = e?.message || 'Please try again later.';
