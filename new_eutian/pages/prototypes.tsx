					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						{filteredPrototypes.length === 0 ? (
							...
							filteredPrototypes.map((prototype, index) => (
								<PrototypeCard key={index} {...prototype} onClick={() => setSelectedPrototype(prototype)} />
							))
						)}
					</div>
				</div>
						...
						<>
							...
							<div className="space-y-6">
								<div className="aspect-video relative overflow-hidden rounded-lg bg-muted">
									<img src={selectedPrototype.image} alt={selectedPrototype.title} className="w-full h-full object-cover" />
								</div>
								<div>
