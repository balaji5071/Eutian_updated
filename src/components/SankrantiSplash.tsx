          ...
          {kitePositions.map((kite, i) => {
            ...
            return (
              ...
              <svg width="40" height="50" viewBox="0 0 40 50" className="drop-shadow-lg">
                ...
                {/* Ribbons */}
                {[...Array(3)].map((_, j) => {
                  const ribbonColors = ['#FF6B6B', '#FFD93D', '#4ECDC4'];
                  const yStart = 30 + j * 5;
                  const yMid = 35 + j * 5;
                  const yEnd = 40 + j * 5;
                  return (
                    <motion.g key={j}>
                      <motion.path
                        d={`M 20 ${yStart} Q 15 ${yMid} 20 ${yEnd}`}
                        stroke={ribbonColors[j]}
                        strokeWidth="2.5"
                        fill="none"
                        opacity="0.9"
                        animate={{
